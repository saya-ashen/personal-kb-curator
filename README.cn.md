...
一个用于**整理、去重、沉淀并持续维护个人知识库/资料库**的 Codex skill 仓库。

它适合处理这类场景：

- 文件夹里堆满了笔记、截图、PDF、草稿、重复版本，想整理成可维护的知识库。
- 已经有一套知识库结构，但会持续接收新资料，需要做**增量归档与合并**。
- 希望 assistant 在回答问题时，**优先检索规范化后的 canonical 资产**，而不是全仓乱扫。
...
如果你的 Codex / agent 环境支持本地 skill 仓库，可以让 assistant 在以下任务中触发它：
